...
contract TestUniswapV3Callee is IUniswapV3MintCallback, IUniswapV3SwapCallback, IUniswapV3FlashCallback {
    ...
    function uniswapV3MintCallback(
        ...
        address sender = abi.decode(data, (address));
        uint256 amount0 = IERC20(IUniswapV3Pool(msg.sender).token0()).balanceOf(sender);
        uint256 amount1 = IERC20(IUniswapV3Pool(msg.sender).token1()).balanceOf(sender);
    ...
    function uniswapV3FlashCallback(
        ...
        if (pay0 > 0) IERC20(IUniswapV3Pool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20(IUniswapV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}
